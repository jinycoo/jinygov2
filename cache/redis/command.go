/**------------------------------------------------------------**
 * @filename redis/command.go
 * @author   jinycoo
 * @version  1.0.0
 * @date     2019-08-02 16:34
 * @desc     go.jd100.com - redis - command
 **------------------------------------------------------------**/
package redis
...
func (cmd *baseCmd) Name() string {
	if len(cmd._args) > 0 {
		...
		s := cstring.ToLower(cmd.stringArg(0))
		cmd._args[0] = s
		return s
	}
	return ""
...
func (cmd *StringCmd) Bytes() ([]byte, error) {
	return cstring.StringToBytes(cmd.val), cmd.err
}
