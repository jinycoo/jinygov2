 ...
 * @desc     go.jd100.com - jiny - metadata
 **------------------------------------------------------------**/
package jiny
...
const (
	// http head
	_httpHeaderUser         = "x1-jd100spy-user"
	_httpHeaderColor        = "x1-jd100spy-color"
	_httpHeaderTimeout      = "x1-jd100spy-timeout"
	_httpHeaderRemoteIP     = "x-backend-jd100-real-ip"
	_httpHeaderRemoteIPPort = "x-backend-jd100-real-ipport"
)

// mirror return true if x1-jd100spy-mirror in http header and its value is 1 or true.
func mirror(req *http.Request) bool {
	mirrorStr := req.Header.Get("x1-jd100spy-mirror")
	if mirrorStr == "" {
		return false
	}
